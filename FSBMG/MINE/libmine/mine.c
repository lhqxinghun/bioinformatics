...
mine_score *mine_compute_score(mine_problem *prob, mine_parameter *param)
{
  ...
  for (i=0; i<score->n; i++)
    {
      ...
      ret = EquipartitionYAxis(yy, prob->n, ynum, Q_map, &q);
      if (ret)
  	    goto error_0;
  ...
  /*Added by Hongqiang Lyu (end)*/
  
  /*Deleted by Hongqiang Lyu (begin)*/
  ///* x vs. y */
  //for (i=0; i<score->n; i++)
  //  {
  //    k = MAX((int) (param->c * (score->m[i]+1)), 1);
