...
spawnTADs_withNesting_dualOutput_origin <- function(ntads, min.tad=10, max.tad=30, nestedLevels=3, nestedProb=0.3, nestedSizeCap=NULL) 
{
  ...
  ####as.integer(),转换变量类型
  ntads <- as.integer(ntads)
  ###runif(),产生随机数 ntads代表个数 min代表最小值 max代表最大值
  mysize <- as.integer(runif(ntads, min.tad, max.tad))
  tads <- list()
  tads_intervals<-NULL
  
  # copy object for subsequent manipulations复制对象
  mysize_ref<-mysize
...
{
  len <- as.integer(len)
  cbind(anchor=1:len, target=1:len)#cbind连接
}
...
spawnLigations2 <- function(nspec, len, diag=FALSE, prior=1, decay=0.5) 
  ...
  #这会添加连接事件，其中所选连接事件的权重为
  #斜向对角线。 这样可以避免在其余部分中投入过多的工作
  #不太可能引起关注的联系矩阵。  
...
aggregateAll <- function(...) 
  ...
  #这需要一堆（坐标矩阵，计数向量）列表，并且合并它们以计算总平均值
{
  everything <- list(...)
...
# x is the squared symmetric contact matrix平方对称接触矩阵
...
runHiCSimulation<-function(BINSIZE, simChromName="ChrS", max.tad=50, min.tad=3,
  ...
  TIME<-(Sys.time()) # can be used to set seed设置种子
  # in this version it is just used to define the output filename 定义输出文件名
  
  ## make this linked to the target chromosome size与目标染色体大小相关
  n.tads <- round((TARGET.chrsize/BINSIZE)/mean(min.tad:max.tad))
  
  ## round()四舍五入取整
  #  signal in diagonal对角线信号
  ...
  for (r in 1:nreplicate) {
    ...
    # Adding some random noise. t添加随机噪音
    ...
    print("Save TADs...")
    if(nreplicate == 1){
      outfilename_tads<-paste(output,"/ChrS_TADs","_", "noisepercent=", percent_of_sampling_points_nonSpecificsignal, "_","overlapprobs=",overlapProbs, "sample_", samplenumber, ".txt", sep="")
    }else if(nreplicate >1){
      outfilename_tads<-paste(output,"/ChrS_TADs","_", "noisepercent=", percent_of_sampling_points_nonSpecificsignal, "_","overlapprobs=",overlapProbs, "sample_", samplenumber, "rep_", r, ".txt", sep="")
    }else{
    ...
    print("Save counts...")
    if(nreplicate == 1){
      chr_filename<-paste(output,"/ChrS_MAT", "_", "noisepercent=", percent_of_sampling_points_nonSpecificsignal, "_","overlapprobs=",overlapProbs, "sample_", samplenumber, ".txt", sep="")
    }else if(nreplicate > 1){
      chr_filename<-paste(output,"/ChrS_MAT", "_", "noisepercent=", percent_of_sampling_points_nonSpecificsignal, "_","overlapprobs=",overlapProbs, "sample_", samplenumber, "rep_", r, ".txt", sep="")
    }else{
      stop("replicate number error")
    }
    ...
    if(nreplicate == 1){
      chr_figurename<-paste(output,"/Matrix_ChrS_figure1_", "noisepercent=", percent_of_sampling_points_nonSpecificsignal, "_","overlapprobs=",overlapProbs,"_","plotrange=",binStart,"_", binEnd, "sample_", samplenumber, ".tiff", sep="")
      chr_figurename2 <- paste(output,"/Matrix_ChrS_figure2_", "noisepercent=", percent_of_sampling_points_nonSpecificsignal, "_","overlapprobs=",overlapProbs,"_","plotrange=",binStart,"_", binEnd, "sample_", samplenumber, ".tiff", sep="")
    }else if(nreplicate > 1){
      chr_figurename<-paste(output,"/Matrix_ChrS_figure_", "noisepercent=", percent_of_sampling_points_nonSpecificsignal, "_","overlapprobs=",overlapProbs,"_","plotrange=",binStart,"_", binEnd, "sample_", samplenumber, "rep_",r, ".tiff", sep="")
      chr_figurename2 <- paste(output,"/Matrix_ChrS_figure2_", "noisepercent=", percent_of_sampling_points_nonSpecificsignal, "_","overlapprobs=",overlapProbs,"_","plotrange=",binStart,"_", binEnd, "sample_", samplenumber, "rep_",r, ".tiff", sep="")
    }else{
      stop("replicate number error")
    }
...
for (overlap in c(0.05, 0.1, 0.15, 0.2, 0.25)) {
  for (noise in c(0.20, 0.04, 0.08, 0.12, 0.16)) {
    outdir <- paste0("/media/disk1/liuerhu/hicda_liuerhu/simdata/overlap/output_final7/overlap",overlap,"/noise",noise*100)
    for (i in 1:nsample) {
      check<-runHiCSimulation(BINSIZE=25000, simChromName="ChrS", max.tad=20, min.tad=5,TARGET.chrsize=110*1e5, base.tad.signal=round(KtBaselineTADs/nestedLevels), interaction.decay.rate=0.45, percent_of_sampling_points_nonSpecificsignal=noise, n.interactions=0,base.diagonal.signal=(KtBaselineTADs*100/80), max.range=(50+round(50*1/3)), base.interaction.signal=(KtBaselineTADs*2),base.nonspecific.signal=round((9/80)*KtBaselineTADs), dispersion=0.001, PNGplot=TRUE, saveVanilla=TRUE,saveDiffHiC=FALSE, saveHOMER=FALSE, addIntConstant=FALSE,nestedTADs=TRUE,nestedLevels=nestedLevels, nestedProb=0.25, nestedSizeCap=75,overlapProbs = overlap, output = outdir, samplenumber = i, nreplicate = nrep)
