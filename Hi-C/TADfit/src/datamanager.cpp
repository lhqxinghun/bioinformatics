...
//winsize >= 2
void DataManager::findiagcpbyTopDom(std::vector<contactmatrix> &sourdataset, int winsize, bool bstatFilter, std::vector<changepoint> &diagcp)
{
    int i, width, height;
    changepoint tcp;
...
void ftrlwrite(std::vector<contactmatrix> &sourdataset, std::vector<contactdomain> &hiercd, int selcdnum, gsl_matrix *&pftrlD, gsl_matrix *pftrlY, gsl_matrix *pftrlX, gsl_matrix *pftrlB)
{
    ...
    //Write Y
    strvalue << "./out/y.out";
    fout.open(strvalue.str(), std::ofstream::out | std::ofstream::trunc); //如果存在，把文件长度设为0
    if(fout.is_open())
    {
    ...
    //Write P
    strvalue << "./out/p.out";
    fout.open(strvalue.str(), std::ofstream::out | std::ofstream::app);     //如果存在，在末尾追加
    if(fout.is_open())
    {
        for(i = 0; i < Bdim; i++)
        {
                             ...
                             int repeatnum, int iteration, double lambda, double alpha, double beta, double l1, double l2)
{
    ...
    double curbvalue;
    std::vector<int> contoidx; //如 [0,0,0,0,1,1,1,1]
    std::vector<std::string> idxtocon; //如[“GM12878”, "IMR90"]
    std::vector<std::pair<int, int>> randcoord;//随机坐标对
    gsl_matrix *pftrlD;//计算X要用到的向量，直接存
    gsl_matrix *pftrlY;//Y (1 * 样本个数）
    gsl_matrix *pftrlX;//X (1 * 候选TAD个数）
    gsl_matrix *pftrlB;//B (候选TAD个数 * context个数）
    gsl_matrix *pftrlZ;//
    gsl_matrix *pftrlN;//
    gsl_matrix *pftrlP;//
    ...
    ftrlalloc(pftrlD, pftrlY, pftrlX, pftrlB, pftrlZ, pftrlN, pftrlP, Ddim, Ydim, Xdim, Bdim);//申请内存，D向量初始化
    for(r = 0; r < repeatnum; r++)//重复遍历
    {
        //clock_t time_r = clock();
	std::cout << "repeat num:" << r << std::endl;
        gsl_matrix_set_zero(pftrlB);//B Z N置为零
	...
	//clock_t time_i = clock();
        ...
        //clock_t time_coorsave = clock();
	//std::cout << "finish coord saving："<< (double)(time_coorsave-time_i)/CLOCKS_PER_SEC << "seconds"<<std::endl;

	for(i = 0; i < iteration; i++)//迭代遍历
        {
            std::cout << "iteration num:" << i << std::endl;
            for(j = 0; j < (int)randcoord.size(); j++)
            {
		
                ftrlgetsample(sourdataset, randcoord.at(j).first, randcoord.at(j).second, hiercd, pftrlD, pftrlY, pftrlX, Xdim);//主要是给X和Y赋值
            ...
            //randcoord.clear();  //我自己加的
            //clock_t time_train = clock();
            //std::cout << "finish traing："<< (double)(time_train-time_coorsave)/CLOCKS_PER_SEC << "seconds"<<std::endl;
            //训练完了把Y和P都写下来
            ftrlwrite(sourdataset, hiercd, Xdim, pftrlD, pftrlY, pftrlX, pftrlB);
            //clock_t time_write = clock();
            //std::cout << "finish file saving："<< (double)(time_write-time_train)/CLOCKS_PER_SEC << "seconds"<<std::endl;
        }
	randcoord.clear();  //我自己加的
	
        /*for(i = 0; i < iteration; i++)//迭代遍历，这块随机化的逻辑有问题?
        {
            std::cout << "iteration num:" << i << std::endl;
	    clock_t time_i = clock();
            for(j = 0; j < height; j++)
        ...
        for(i = 0; i < Xdim; i++)
        {
            for(j = 0; j < Bdim; j++)
            {
                curbvalue = fabs(gsl_matrix_get(pftrlB, i, j)); //把beta值写入到hiercd中去。
